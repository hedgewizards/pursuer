        ...
        try
        {
            ...
            if (ilist.Count > 0)
                ...
                {//Can use UnityObject.Name
                    ...
                    stringList.AddRange(arr);
                    if (SelectedID == -1 && stringList.Count > 0)
                    {
                        // If we couldn't find this item in our list of strings, add it as a new string at the end
                        stringList.Add("<other>");
                        SelectedID = stringList.Count - 1;
                    }
                    //int newSelectedID = EditorGUILayout.Popup(dropdown, SelectedID, arr);
                    int newSelectedID = EditorGUI.Popup(position, property.name, SelectedID, stringList.ToArray());
                ...
                {//Use SystemObject.ToString()
                    ...
                    List<string> stringList = new List<string>();
                    stringList.AddRange(ListToStringArray(SystemObjectList, GetItemName, BaseMaster));
                    if (SelectedID == -1 && stringList.Count > 0)
                    {
                        // If we couldn't find this item in our list of strings, add it as a new string at the end
                        stringList.Add("<other>");
                    ...
                    int newSelectedID = EditorGUI.Popup(position, property.name, SelectedID, stringList.ToArray(), GetDropdownStyle(Color.blue));
                    if (newSelectedID != SelectedID)
                    {//changed
                        SelectedID = newSelectedID;
